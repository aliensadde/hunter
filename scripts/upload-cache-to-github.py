...
class Cache:
    ...
    for root, dirs, files in os.walk(cache_dir):
      for filename in files:
        if filename == 'CACHE.DONE':
          entries.append(CacheEntry(os.path.join(root, filename), cache_dir))
  ...
  def try_to_push(self, main_remote, main_remote_url_pull, github):
    ...
    try:
      ...
      # Do not print exception, avoid leakage of token
      print("Push failed")
      return False
  ...
  def upload_meta(self, github, cache_dir):
    cache_meta = os.path.join(cache_dir, 'meta')
    repo = git.Repo.init(cache_meta)
    ...
    if repo.is_dirty(untracked_files=True):
      ...
      repo.index.commit(self.make_commit_message())
    ...
    if not main_remote_found:
      main_remote = repo.create_remote('origin', main_remote_url)

    main_remote.fetch()
    repo.heads.master.set_tracking_branch(main_remote.refs.master)
